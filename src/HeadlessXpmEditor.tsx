import React, { useEffect } from "react";
import type { ReactNode, CSSProperties } from "react";
import { useHeadlessXpmContext } from "./HeadlessXpmProvider";
import './HeadlessXpmEditor.css';

export interface HeadlessXpmEditorProps {
    tcmId: string;
    children: ReactNode
    isPage?: boolean;
    className?:string
    linkStyle?: CSSProperties;
    containerStyle?: CSSProperties;
    contentStyle?: CSSProperties;
    iconStyle?: CSSProperties;
}

export const HeadlessXpmEditor = ({ tcmId, children, isPage = false,className, containerStyle, contentStyle, iconStyle, linkStyle }: HeadlessXpmEditorProps): JSX.Element => {
    const { editorUrl, icon, showExpSpaceEditor, toggleXpm, setPageId, setShowPage, staging } = useHeadlessXpmContext();

    const link = isPage ? `${editorUrl}/page?item=${tcmId}&tab=general.content` : `${editorUrl}/component?item=${tcmId}&tab=general.content`;

    useEffect(() => {
        if (isPage) {
            setShowPage(true);
            setPageId(tcmId)
        }
    }, [isPage, tcmId]);
    
    if (!staging) return <>{children}</>;

    return (
        <div
            className={`headlessXpmRegion ${className || ""} ${toggleXpm || !showExpSpaceEditor ? "headlessXpmRegion-hover" : ""}`} style={{ ...containerStyle } as React.CSSProperties}
        >
            {(toggleXpm || !showExpSpaceEditor) &&
                <a
                    href={link}
                    className="headlessXpmIcon"
                    target="_blank"
                    rel="noopener noreferrer"
                    title="Edit in Experiance sapce"
                    style={{
                        ...linkStyle,
                        transition: "opacity 0.3s ease",
                    } as React.CSSProperties}
                >
                    {
                        icon ? icon :
                            <svg style={{...iconStyle}} viewBox="64 64 896 896" focusable="false" data-icon="edit" width="1em" height="1em" fill="currentColor" aria-hidden="true">
                                <path d="M257.7 752c2 0 4-.2 6-.5L431.9 722c2-.4 3.9-1.3 5.3-2.8l423.9-423.9a9.96 9.96 0 000-14.1L694.9 114.9c-1.9-1.9-4.4-2.9-7.1-2.9s-5.2 1-7.1 2.9L256.8 538.8c-1.5 1.5-2.4 3.3-2.8 5.3l-29.5 168.2a33.5 33.5 0 009.4 29.8c6.6 6.4 14.9 9.9 23.8 9.9zm67.4-174.4L687.8 215l73.3 73.3-362.7 362.6-88.9 15.7 15.6-89zM880 836H144c-17.7 0-32 14.3-32 32v36c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-36c0-17.7-14.3-32-32-32z"></path>
                            </svg>
                    }
                </a>
            }
            <div className="headlessXpmContent" style={{ ...contentStyle }}>
                {children}
            </div>
        </div>
    );
};

